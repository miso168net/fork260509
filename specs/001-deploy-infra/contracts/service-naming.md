...
**Consumers**: nginx conf（反代 target）、new-admin-rust-api 環境變數（DATABASE_URL / REDIS_URL）、migration（depends_on）
...
範例（從 new-admin-rust-api container 內看）：
```bash
...
**契約**：service name `new-admin-rust-api` + port `10001`，end of `/api/` 後的路徑以 `/` 為起點傳給上游（strip `/api/` 前綴）。

### 2. new-admin-rust-api → postgres
...
**契約**：跟 new-admin-rust-api 用同一個 DATABASE_URL pattern；只跑一次（`restart: "no"`）。
...
2. 同步修改所有 consumer：
   ...
   - new-admin-rust-api 環境變數（DATABASE_URL / REDIS_URL）
   - 任何 docs / quickstart 引用
3. PR 必須在 title 標 `BREAKING:` 前綴。

例：未來若 `new-admin-rust-api` 改為 `admin-api-server`，需 nginx + new-admin-rust-api env + docs 三處同步。
...
for svc in postgres redis migration new-admin-rust-api new-admin-base-web; do
...
grep -E 'proxy_pass http://new-admin-rust-api:10001' deploy/nginx/default.conf \
  && echo "PASS: nginx → new-admin-rust-api:10001" \

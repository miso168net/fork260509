...
| `*` | `/api/<rest>` | reverse proxy | `proxy_pass http://new-admin-rust-api:10001/<rest>`（strip `/api/` 前綴） | 不 cache（API 預設） |
...
`/api/` location 必須注入以下 header 給上游 new-admin-rust-api：
...
**為何**：
- `Host` / `X-Real-IP` / `X-Forwarded-*`：讓 new-admin-rust-api 能拿到原 client IP（log / audit / rate limit 用）。
...
- 本架構是**同源**部署（瀏覽器 → nginx :8080 → 內網 new-admin-rust-api:10001），瀏覽器只看到一個 origin → 根本不會發 CORS preflight。
...
**陷阱**：若有 `/api` 路徑開頭的「真實檔案」（罕見），會被 `/api/` location 蓋。本契約不允許 new-admin-base-web image 內 `dist/api/*` 出現任何檔（feature 7 admin-web-dockerfile 須確保 build 產出無此路徑）。
...
**用途**：
1. docker HEALTHCHECK：`wget -qO- http://localhost/health || exit 1`（new-admin-base-web image Dockerfile 內，feature 7 處理）
...
grep -qE 'proxy_pass http://new-admin-rust-api:10001/?$' deploy/nginx/default.conf \
  && echo "PASS: proxy_pass 指向 new-admin-rust-api:10001" \

...
| ID | 範疇 | 處理方式 | 備註 |
|---|---|---|---|
| R-clippy-ptr_arg | admin-api / server-utils | ✅ **已修**（admin-api inner `42ff2ca`） | `admin-api/server/utils/src/tree_util.rs:180` `nodes: &mut Vec<T>` → `&mut [T]`；function body 只用 iter_mut()、call sites auto deref-coerce no caller change |
| R-clippy-lifetimes | admin-api / sea-orm-adapter | ✅ **已修**（admin-api inner `42ff2ca`） | sea-orm-adapter/src/action.rs 7 個 fn 的 `<'conn, 'rule>` 全 elide（lines 46/61/71/101/129/138/150；其中 remove_filtered_policy 只 elide 'conn 因 'rule 跨兩 param 不能 elide）。Bonus：順手修 sea-orm-adapter/src/adapter.rs:405 `vec![String::new(); 0]` → `Vec::<String>::new()`（clippy::zero_repeat_side_effects） |
| R-axum-version-conflict | admin-api / axum-casbin tests | ✅ **已修**（admin-api inner `45bd89c`） | 採 axum-test-helpers → axum-test 20 migration（同維護者 JosephLenton 的新 crate、對應 axum 0.8 era）。workspace Cargo.toml + axum-casbin/Cargo.toml + server/initialize/Cargo.toml dep 改、axum-casbin/tests/{test_middleware,test_middleware_domain,test_set_enforcer}.rs API migration（`TestClient → TestServer`、`/book/:id → /book/{id}` axum 0.8 syntax、`.status() → .status_code()`）。Cargo.lock 現在 only one axum (0.8.9) + one axum-core (0.5.6)；3 個 test 全 pass |
| R-pnpm-flatten | admin-web / pnpm 11 monorepo | ✅ **已修**（admin-web inner `72ff8378`） | Discovery: pnpm 11 不再讀 `.npmrc` 的 `public-hoist-pattern`（已 migrated to pnpm-workspace.yaml；shamefully-hoist + link-workspace-packages 也已 no-op）。採 approach (a) 直接加 4 個 explicit deps 到 root package.json（axios@1.15.0 + @iconify/utils@3.1.0 + @unocss/core@66.6.8 + @unocss/preset-mini@66.6.8，版本對齊 lock 既有 transitive 解析、非升版）。vue-tsc 15 errors → 0 errors，vite build PASS。Side effect: pnpm-workspace.yaml allowBuilds + src/typings/components.d.ts auto-regen |
| R-eslint-v10-migration | admin-web / eslint config | ✅ **false positive 確認**（無需 fix） | 重 verify：admin-web 早就有 `eslint.config.js`（254 bytes、import @soybeanjs/eslint-config-vue、flat config 格式）。earlier `npx eslint .` fail 只是 wrong cwd（從 outer 跑而非 admin-web）。從正確 cwd 跑 EXIT=0，0 errors。R5 entry retire |

> 註：以上 5 條都不是 features 1-8 引入的；F8 acceptance evidence 與 spec / code reviews 全 PASS（F8 modified file 0 命中在任何 clippy / test 錯誤訊息）。
>
> **2026-05-12 全部處理完畢**：4 條真實問題已修（R-clippy-ptr_arg / R-clippy-lifetimes / R-axum-version-conflict 全 admin-api inner、R-pnpm-flatten admin-web inner）、1 條（R-eslint-v10-migration）為 false positive 已 retire。Host-side `cargo clippy` / `cargo test --workspace` / `pnpm typecheck/build/eslint` 全 PASS。
